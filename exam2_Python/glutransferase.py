...
def getpeptides(dns_sequence):
    coding_dna = translate_sequence(dna_sequence)
    peptides = []
    for i in range(len(coding_dna)):
        peptides = coding_dna[i:]
        if peptides.startwith("M"):
...
def Sequences(accessions):
    ...
    for accession in accessions:
        ...
        peptides = generate_pepptides(dna_sequence)
    ...
    g.fig.subtitle("Peptide Molecular Weight vs Instability Index")
    plt.savefig("results/glupeptides.png")
    plt.close()
    
    
    all_data.to.csv("results/glupeptides.csv", index=False)
